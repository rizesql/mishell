...
impl TraceEventConfig {
    ...
    pub fn all() -> Self {
        Self::init(&[
            TraceEvent::Arithmetic,
            TraceEvent::Commands,
            TraceEvent::Complete,
            TraceEvent::Expand,
            TraceEvent::Functions,
            TraceEvent::Jobs,
            TraceEvent::Parse,
            TraceEvent::Pattern,
            TraceEvent::Tokenize,
    ...
    fn reload_filter(&mut self) -> Result<(), Error> {
        if let Some(handle) = &self.handle {
            if handle.reload(self.compose_filter()).is_ok() {
                ...
                Err(Error::Unimplemented("failed to enable tracing events"))
            }
        } else {
            Err(Error::Unimplemented("tracing not initialized"))
        }
    }
}
